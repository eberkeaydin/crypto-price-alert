...
import { createServer, Server } from 'http'; // ✅ Server tipini import et
import handler from '../../pages/api/alert';
import { NextApiRequest, NextApiResponse } from 'next';

let server: Server; // ✅ `server` değişkenine açıkça `Server` tipi ekledik
...
describe('Integration - Alert API', () => {
  test('POST /api/alert - should create and retrieve an alert', async () => {
    await request('http://localhost:3001')
      .post('/api/alert')
      .send({ userId: '123', crypto: 'BTC', condition: 'greater_than', targetPrice: 50000 });
